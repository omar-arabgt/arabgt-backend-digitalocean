...
def send_push_notification(user_id, title, content, link=None, external_link=None, by_admin=False):
    ...
    if user_id == NOTIFICATION_ALL:
        ...
        user_filter = {"filters": [{"field": "tag", "key": "send_notification", "relation": "=", "value": "true"}]}
    else:
        users = User.objects.filter(id=user_id, send_notification=True)
        user_filter = {"include_external_user_ids": [str(user_id)]}
...
@shared_task
def sync_send_notification_tag(user_id, send_notification):
    url = f"https://api.onesignal.com/apps/{settings.ONESIGNAL_APP_ID}/users/by/external_id/{user_id}"
    payload = {
        "properties": {
            "tags": {"send_notification": str(send_notification).lower()},
        }
    }
    headers = {
